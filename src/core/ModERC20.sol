...
contract ERC20 is Ownable, IERC20 {
    ...
    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
                                // v this allows msg.sender to be a 3rd party that is given allowance
        _spendAllowance(from, _msgSender(), amount); // then msg.sender can transfer tokens from => to 
        _transfer(from, to, amount); // without requiring from to pay gas fees
        return true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        if(fromBalance < amount) revert InvalidTransferAmount();
        unchecked {
